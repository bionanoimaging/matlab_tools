...
// gparams=global parameters: [0]: global offset
// params: blocks of size paramSize. One block for each particle
// [0]: brightness (pre-exponential factor)
// [1]: width of the Gaussian
// [1 .. paramSize-1]: position along the dimension
   ...
   prhs # right handed parameter array*/
    ...
    else
    {
        ...
        if (mydata != 0) {
            ...
            int d,n;
            for (d=0;d<PsizeY;d++)   // to account for center is zero
            for (n=0;n<PsizeX;n++)   // to account for center is zero
                if (n>0)
