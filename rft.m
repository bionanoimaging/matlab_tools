...
firstTrans = find(transformDirs,2,'first');
if isa(in,'dip_image') && firstTrans(1)==1
    firstTrans=firstTrans(2);
else
    firstTrans=2;
end
...
if isa(in,'dip_image')     if nargin < 1
        b=b/sqrt(prod(size(b)));
    else
        validSizes=size(b) .* transformDirs2(1:ndims(b));
