%de=DampEdge (im,part,dim,shape,method)
%
%dims edges of the input image - Hann window with with central flat are
%
%INPUT:
...
%method - 1: dim down to zero, 2: fade to a Gaussian blurred average of the opposing lines
%sigma : Size of the Gaussian blurring kernel for method 2, (default: 10)

function [de,noiseFactor]=DampEdge (im,part,dim,shape,method,sigma)
if nargin<2
    part=0.15; % dim at 15%
end
if nargin<3
...
elseif method == 2 
    ...
    if shape == 0
        error('Mirroring is only allowed for square shape');
    end
    ...
    if ndims(im) == 3 && size(im,3) > 1
        ...
        if (dim > 1)
            ...
            if (border > 1)
                w=xx([border,size(im,2),size(im,3)],'corner')/(border-1);  % make a linear weigth from left to right
            else
                w=xx([border,size(im,2),size(im,3)],'corner');  % make a linear weigth from left to right
            end
            ...
            tmp=im2(0:border-1,:,:)+Avg.*(1-w); 
            im2(0:border-1,:,:)=tmp;
            tmp=sumw(0:border-1,:,:)+(1-w);
            sumw(0:border-1,:,:)=tmp;
            tmp=max(maxw(0:border-1,:,:),(1-w));
            maxw(0:border-1,:,:)=tmp;
        ...
        if (dim >= 2)
            ...
            if (border > 1)
                w=yy([size(im,1),border,size(im,3)],'corner')/(border-1);  % make a linear weigth from left to right
            else
                w=yy([size(im,1),border,size(im,3)],'corner');  % make a linear weigth from left to right
            end
            w=(1-cos(w*pi))/2;   % cos ^2 weighting
        ...
        if (dim >= 3) && (size(im,3) > 1)
            ...
            if (border > 1)
                w=zz([size(im,1),size(im,2),border],'corner')/(border-1);  % make a linear weigth from left to right
            else
                w=zz([size(im,1),size(im,2),border],'corner');  % make a linear weigth from left to right
            end
            w=(1-cos(w*pi))/2;   % cos ^2 weighting
        ...
        im2(sumw>0)=im2(sumw>0)./sumw(sumw>0);
        de=im2*maxw+im*(1-maxw);
    elseif ndims(im)<=3 
        if (dim == 1)
            border=ceil(size(im,1)/2*(part/2));
            if (border > 1)
                w=xx([border,1],'corner')/(border-1);  % make a linear weigth from left to right
            else
                w=xx([border,1],'corner');  % make a linear weigth from left to right
            end
            w=(1-cos(w*pi))/2;  % cos ^2 weighting
            Avg=gaussf(im);
            Avg=(Avg(0)+Avg(end))/2;
        ...
        elseif (dim > 1)
            border=ceil(size(im,1)/2*(part/2));
            if (border > 1)
                w=xx([border,size(im,2)],'corner')/(border-1);  % make a linear weigth from left to right
            else
                w=xx([border,size(im,2)],'corner');  % make a linear weigth from left to right
            end
            w=(1-cos(w*pi))/2;  % cos ^2 weighting
        ...
        if (dim >= 2)
            border=ceil(size(im,2)/2*(part/2));
            if (border > 1)
                w=yy([size(im,1),border],'corner')/(border-1);  % make a linear weigth from left to right
            else
                w=yy([size(im,1),border],'corner');  % make a linear weigth from left to right
            end
            w=(1-cos(w*pi))/2;   % cos ^2 weighting
            Avg=repmat(gaussf(squeeze(im(:,0)/2+im(:,end)/2),sigmaX),[1 border]);
            tmp=im2(:,0:border-1)+Avg.*(1-w); 
            im2(:,0:border-1)=tmp;
            tmp=sumw(:,0:border-1)+(1-w);
            sumw(:,0:border-1)=tmp;
